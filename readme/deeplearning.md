...
基于任务三，我重新选择搭建了简化的 ResNet18 模型，通过我在互联网上的学习下，我了解到：ResNet 相比 VGG 更适配 CIFAR-10，核心在于其结构设计更贴合 32×32 小尺寸图片的特征提取需求：通过残差连接缓解了深层网络的梯度消失问题，；用卷积步长替代显式池化，避免了小图片信息过早丢失；同时批量归一化和更合理的参数分布，使其在仅有 5 万训练样本的 CIFAR-10 上不易过拟合，配合 MixUp 等数据增强策略时泛化能力更强，最终准确率和训练稳定性均显著优于 VGG。
##### 总体思路
在报告结果之前，先来聊一聊我在训练 ResNet18 模型时学习到的一些知识。
VGG 学习的是 “输入到输出的直接映射”，易记住 CIFAR-10 训练集中的噪声（如图片的随机像素、标注误差）；ResNet 学习的是 “输入与输出的残差（差值）”，更关注图片的核心特征（如 “猫的耳朵”“飞机的机翼”）而非噪声，即使测试集样本与训练集存在细微差异，仍能准确分类，泛化能力显著优于 VGG。
在了解到 ResNet 的这些优势后，我决定尝试用 ResNet18 模型来训练 CIFAR-10 数据集。首先我根据根据任务三的代码，重新搭建了 ResNet18 模型，此时我没有加入其他代码，接着便直接开始训练模型。
很快，模型就开始训练了。我选择了 Adam 优化器，损失函数选择了交叉熵损失。在训练过程中，我注意到模型的损失在不断下降，同时验证集上的准确率也在不断提高。这说明模型正在学习到有用的特征表示，并且能够对新数据进行准确分类。
在task004中的test01.ipynb的日志中，与task003中的test01.ipynb可以明显看到模型的准确率提高了十个百分点。这说明 ResNet18 模型在 CIFAR-10 数据集上的表现要优于 VGG 模型。
不过，我也发现了他们两个都存在一个巨大的问题，那就是过度拟合。例如下面的图片中，task003中VGG模型的训练损失和验证准确率曲线，与task004中ResNet18模型的训练损失和验证准确率曲线对比起来，两者的曲线在训练集上的损失不断下降，而在验证集上的准确率则在某个 epoch 之后开始下降(震荡)，这说明模型在训练集上过度拟合，而在验证集上泛化能力较差。
![](image_004_01.png)
<center><strong>Figure task003-test01</strong></center>

![](image_004_02.png)
<center><strong>Figure task004-test01</strong></center>

因此，我需要对模型进行优化，以防止过度拟合。
首先，我在网上了解到AdamW优化器是Adam优化器的一个变种，它在Adam的基础上加入了权重衰减（L2正则化）。这一变种在处理大规模数据集时表现更好，能够有效防止过拟合。AdamW 修复了 Adam 的权重衰减缺陷，权重衰减独立且稳定，泛化能力更强，是大模型、正式训练的首选。所以我将代码中的 Adam 换成了 AdamW。
同时，我使用了余弦退火学习率调度器，这是一种常用的学习率调度策略，能够在训练过程中动态调整学习率，提高模型的收敛速度和泛化能力。防止学习率过高导致模型训练不稳定，同时也能够在训练后期加速模型收敛。这一点与VGG模型的学习率调度策略不同，VGG模型使用的是固定学习率，而余弦退火学习率调度器则能够根据训练进度动态调整学习率，从而更好地适应模型的训练过程。就好比VGG会学习到图片的无关信息，而使用余弦退火之后随着学习率的降低，无关信息会逐渐被忽略，从而更关注有关信息。
![](image_004_03.png)
<center><strong>Figure task004-test02</strong></center>

很明显，在加入AdamW优化器和余弦退火学习率调度器之后，模型的训练损失和验证准确率曲线都有了明显的改进。训练损失在不断下降，而验证准确率则在不断提高，这说明模型正在学习到有用的特征表示，并且能够对新数据进行准确分类。

那么，准确率能否再提高呢？在任务要求的提示下，我选择加入 MixUp 数据增强策略和 Warm Up 学习率策略。
MixUp 是一种数据增强技术，它通过线性组合不同样本的特征和标签，生成新的样本。这一技术能够有效增加训练样本的数量，同时也能够提高模型的泛化能力。在我的代码中，我将 MixUp 数据增强策略加入到了训练过程中。
Warm Up 学习率策略是一种常用的学习率调度策略，它在训练初期使用一个较小的学习率， gradually 增加到预设的学习率。这一策略能够帮助模型更快地收敛，并且避免在初始阶段学习率过高导致的训练不稳定问题。在task004中的test03的代码中，我将 Warm Up 学习率策略加入到了训练过程中。
![alt text](image_004_04.png)
<center><strong>Figure task004-test03</strong></center>

可以看到，加入 MixUp 数据增强策略和 Warm Up 学习率策略之后，模型的训练损失和验证准确率曲线都有了明显的改进。训练损失在不断下降，而验证准确率则在不断提高，这说明模型正在学习到有用的特征表示，并且能够对新数据进行准确分类。
###### AI 使用说明
AI 最有用的环节：本次AI不仅在绘图方面给予了我最大的帮助，同时在我自主学习ResNet18模型的过程中，也给了我很多启发。例如，我在学习ResNet18模型的过程中，了解到了ResNet模型的核心思想，即通过残差连接缓解了深层网络的梯度消失问题，从而能够训练更深的模型。我还学习到了ResNet模型的结构设计，例如用卷积步长替代显式池化，避免了小图片信息过早丢失；同时批量归一化和更合理的参数分布，使其在仅有 5 万训练样本的 CIFAR-10 上不易过拟合，配合 MixUp 等数据增强策略时泛化能力更强。
AI 未失效的环节：本次任务中 AI 未出现失效情况

## 任务五

初次接触实例分割的任务，我选择围绕 Mask R-CNN 模型实现 PennFudanPed 行人数据集的实例分割
##### 总体思路：
首先自动下载并加载行人数据集，将得到的图片和掩码进行分类之后，提取每个行人实例掩码，这一步类似PS抠图，我从掩码中获取行人的掩码像素值（1：），去除背景（0），接着为每个行人创建的独立选区通道，将一维的 obj_ids 数组扩展为三维，然后与二维的 mask 进行逐元素比较，最终生成一个形状为 (num_objs, H, W) 的布尔数组。然后生成边界框，并转换为模型所需的 Tensor 格式。

接着构建包含张量转换、随机水平翻转的预处理流水线，区分训练和测试阶段的增强策略。这一步跟任务三任务四的预处理十分相似，就不再详细说明。

接着来到加载预训练的 Mask R-CNN 模型。根据我在网上的学习中，我了解到我用的是一个在 COCO 数据集上预训练好的 Faster R-CNN 模型，这个模型有91个类别，远远超过我所需的 “背景 + 行人” 2个类别的需求，所以，我需要对模型进行微调，将其类别数从91个类别减少到2个类别。这样就完成了模型头部的适配，让预训练模型能够专注于行人检测与分割任务。
随后配置任务四同款 AdamW 优化器和并且加入学习率调度器，在训练循环中完成模型的前向传播、损失计算与反向更新，并保存训练后的模型权重；最后加载测试图片，用训练好的模型进行推理，过滤低置信度预测结果，通过可视化将边界框、掩码叠加到原图上，直观展示行人实例分割效果。
![alt text](image_005_01.png)
<center><strong>Figure task005-task005</strong></center>

###### AI 使用说明
AI 最有用的环节：本次AI在实例分割任务中给予了我最大的帮助，同时在我自主学习 Mask R-CNN 模型的过程中，也给了我很多启发。例如，我在学习 Mask R-CNN 模型的过程中，了解到了 Mask R-CNN 模型的核心思想，即通过在 Faster R-CNN 模型的基础上加入掩码预测分支，实现了对每个实例的像素级分割。
AI 未失效的环节：本次任务中 AI 未出现失效情况